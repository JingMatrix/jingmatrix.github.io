...
let origin = "https://www.math.univ-toulouse.fr/~jma";
if (env.GITHUB_PAGES == "true") origin = "https://jingmatrix.github.io";
if (env.NETLIFY == "true") origin = "https://jianyu-ma.netlify.app";
if (env.RENDER == "true") origin = "https://jianyu-ma.onrender.com";
if (env.GITLAB_CI == "true" && env.CI_PAGES_URL != undefined)
  origin = env.CI_PAGES_URL;

let base = new URL(origin).pathname;
...
function fixLink(item: Object): boolean {
  // Fix the bug of base dir for non-html links
  if (
    "link" in item &&
    typeof item.link == "string" &&
    item.link.startsWith("/") &&
    item.link.includes(".") &&
    !item.link.startsWith(base)
  ) {
    item.link = base + item.link.slice(1);
    return true;
  }
  return false;
}

export { vue, markdown, manifest, origin, base, fixLink };
