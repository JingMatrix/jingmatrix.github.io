<template>
	...
	<div v-for="feature in features.content" :key="feature.title" class="cards flex items-start">
		<div class="w-12 flex-shrink-0 flex flex-col text-center leading-none">
			...
			<span class="font-medium text-xl text-gray-800 title-font dark:text-gray-500">{{
			getDay(feature.date)
			}}</span>
		</div>
		<div class="pl-6 relative">
			...
			<blockquote class="leading-relaxed mb-5 text-gray-600 dark:text-gray-400 whitespace-pre-wrap">{{ feature.excerpt }}</blockquote>
			<div v-if='feature.author != "Jianyu MA"' class="inline-flex items-center absolute right-0">
				<img alt="blog" :src="feature.img"
