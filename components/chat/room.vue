<template>
	<div v-if="loading" class="mt-20 relative flex flex-col items-center">
		<img :src="withBase('/img/loading.svg')" class="w-screen max-w-xl" />
	...
	<div v-else class="relative flex flex-col items-center bg-gray-500" id="chat-room">
		...
		<Video />
		<History :messageList="chatData.messageList" :systemMsg="chatData.systemMsg" :name="chatData.name" @reply-msg="(msg) => {
			chatData.replyMsg = msg;
		}
			" />
		<Input :msgcolor="chatData.msgcolor" @send-message="(msg) => {
			send_message(socket, msg, chatData);
		}
			" />
	</div>
</template>
