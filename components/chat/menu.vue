<template>
	<div class="flex rounded-lg top-0 flex-row justify-between h-12 w-full max-w-2xl px-4 py-3 fixed" id='chat-room'
		...
		<span class="flex-grow text-xl text-center" :style="{ color: msgcolor }">
			{{ replyMsg.name? $frontmatter.replyTitle + replyMsg.name : $frontmatter.online + userList.join(', ') }}
		</span>
	...
	<div v-if="config"
		...
		<input v-model="name" :placeholder="name" :style="{ color: namecolor }" type="text"
			class="mx-5 my-5 border-b-2 py-1 border-solid" @change="$emit('change-name', name)" />
...
defineEmits(['video-chat', 'subscribe', 'change-name', 'msg-color', 'name-color'])
